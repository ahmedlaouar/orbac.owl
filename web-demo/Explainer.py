...
class Explainer:
    ...
    def load_from_json(self):
        try:
            filename = "web-demo/tmp/best_params_" + self.model_name.split("/")[-1] + ".json"
    ...
    def optimize_model(self, n_trials, policy):
        ...
        file_name = "web-demo/tmp/best_params_" + self.model_name.split("/")[-1] + ".json"
    ...
    def interactive_prompting(self, evaluator, access):
        ...
        logger.debug('Trials.')
        trial = 0
        while trial < 4 and (coverage < PARAMS_THRESHOLD or correctness < PARAMS_THRESHOLD or clarity < PARAMS_THRESHOLD) :
            try:
                if trial > 0:
                    user_prompt = IMPROVE
                ...
                csv_file_path = "web-demo/tmp/output_{}.csv".format(self.model_name.split("/")[-1])
    ...
    def few_shot_prompting(self, evaluator, access):
        ...
        try:
            ...
            csv_file_path = "web-demo/tmp/few_shot_output_{}_updated.csv".format(self.model_name.split("/")[-1])
            file_exists = os.path.isfile(csv_file_path)
            # Write to CSV without using pandas
            with open(csv_file_path, mode="a", newline="", encoding="utf-8") as file:
                writer = csv.DictWriter(file, fieldnames=output.keys(), delimiter=";")                
