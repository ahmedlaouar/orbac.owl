...
def check_acceptance_query(policy, access):
    acceptance_query_path = "queries.sparql/new_acceptance_query.sparql" # "queries.sparql/acceptance_query.sparql" # 
    with open(acceptance_query_path, 'r') as file:
        query_template = file.read()

    query = query_template.format(example_uri=policy.example_uri, subject=access.subject, action=access.action, object=access.obj)
    results = policy.graph.query(query)
    try:
        first_result = next(iter(results))
        if first_result:
            return True
